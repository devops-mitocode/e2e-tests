...
//                    sh 'sleep 2m'
                    ...
                    sh "docker run --rm -v ${WORKSPACE}:/usr/src/app -w /usr/src/app --name ${BUILD_TAG} --network ${BUILD_TAG}_default maven:3.8.8-eclipse-temurin-17 mvn clean verify -Dwebdriver.remote.url=http://${BUILD_TAG}-selenium-hub-1:4444/wd/hub -Dwebdriver.remote.driver=${BROSWER} -Dchrome.switches=\"--user-agent=ngrok-skip-browser-warning\" -Denvironment=${ENVIRONMENT} ${cucumberFilterTags} -B -ntp"
